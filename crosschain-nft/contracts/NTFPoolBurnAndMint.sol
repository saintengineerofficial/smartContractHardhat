...
contract NTFPoolBurnAndMint is CCIPReceiver, OwnerIsCreator {
    ...
    // 声明请求数据结构体，用于解码跨链消息
    // 从原链发送过来的数据，需要解码成RequestData结构体
    struct RequestData{
        uint256 tokenId;
        address newOwner;
    }
    ...
    // 1. tokenId:你要跨链发送的 NFT 是哪一个？这是基础标识。
    // 2. newOwner:这个地址是目标链的最终用户地址。
    // 假如 Alice 想把她在以太坊上的 NFT 跨到 Polygon，newOwner 就是她在 Polygon 上的钱包地址。
    // 3. chainSelector:告诉桥/中继系统：“我要跨到哪条链”。
    // 4. receiver:目标链上接收这笔 NFT 的合约地址，负责铸造或释放 NFT。目标链上的合约会收到这条消息，然后调用 mint 或 release 给 newOwner。

    function burnAndSendNFT(uint256 tokenId,address newOwner,uint64 chainSelector,address receiver) public 
    returns (bytes32) {
        // transfer nft to this address to burn and send
